...
export const vec3 = {
  ...
  normalize(v: F32Arr) {
    ...
    if (length > 1e-5) {
      ...
      v[1] /= length
      v[2] /= length
    } else throw new Error('cannot normalize zero vector')
  },

  dot(a: F32Arr, b: F32Arr) {
    return a[0] * b[0] + a[1] * b[1] + a[2] * b[2]
  },

  cross(a: F32Arr, b: F32Arr) {
  ...
  triple(a: F32Arr, b: F32Arr, c: F32Arr) {
    return this.dot(a, this.cross(b, c))
  },

  // returns rotation matrix for axis a and angle
  rotationMatrix(a: F32Arr, angle: number): F32Arr {
  ...
  applyRotation(R: F32Arr, v: F32Arr) {
...
export const mat4 = {
  multiply(a: F32Arr, b: F32Arr, o: F32Arr) {
    const a00 = a[0]
    const a01 = a[1]
    const a02 = a[2]
    const a03 = a[3]
  ...
  multiplyVector(a: F32Arr, v: F32Arr) {
  ...
  lookAtAndViewMatrix(eye: F32Arr, target: F32Arr, x: F32Arr, l: F32Arr, v: F32Arr) {
    ...
    v[4] = x[1]
    v[8] = x[2]
    v[12] = -x[0] * eye[0] - x[1] * eye[1] - x[2] * eye[2]
    v[1] = y[0]
    v[5] = y[1]
    v[9] = y[2]
    v[13] = -y[0] * eye[0] - y[1] * eye[1] - y[2] * eye[2]
    v[2] = z[0]
    v[6] = z[1]
    v[10] = z[2]
    v[14] = -z[0] * eye[0] - z[1] * eye[1] - z[2] * eye[2]
    v[15] = 1
  },
}
