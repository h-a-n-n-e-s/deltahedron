...
export class Camera {
  ...
  private lookAtMatrix!: F32Arr
  private viewMatrix!: F32Arr
  private viewProjectionMatrix!: F32Arr
  private eye!: F32Arr
  ...
  constructor(para: CameraParameters) {
    this.para = para
  }

  shaderParameterMapping(shaderParameters: F32Arr) {
    this.lookAtMatrix = shaderParameters.subarray(0, 16) as F32Arr
    this.viewMatrix = shaderParameters.subarray(16, 32) as F32Arr
    this.viewProjectionMatrix = shaderParameters.subarray(32, 48) as F32Arr
    this.eye = shaderParameters.subarray(48, 51) as F32Arr // eye is initialized here!
  ...
  getCameraMatrix() {
    ...
    if (this.para.arcRotateCamera) this.sphericalToCartesian()
    else {
      ...
      for (let o = 0; o < 3; o++) this.eye[o] *= this.para.radius / l
    }

    mat4.lookAtAndViewMatrix(
      this.eye,
      this.para.target,
      this.tx,
      this.lookAtMatrix,
  ...
  getMouseRay() {
    ...
    const cameraCoords = mat4.multiplyVector(
      this.inverseProjection,
      new Float32Array([this.mouseCoords.x, this.mouseCoords.y, -1, 0])
    )
  ...
  keyboardInteraction(canvas: HTMLCanvasElement) {
    ...
    canvas.addEventListener('keydown', (e) => {
      if (e.key === 'ArrowLeft') {
        ...
        if (this.para.azimuth === -this.para.angleResolution)
          this.para.azimuth = 360 - this.para.angleResolution
      } else if (e.key === 'ArrowRight') {
        this.para.azimuth += this.para.angleResolution
        if (this.para.azimuth === 360) this.para.azimuth = 0
      } else if (e.key === 'ArrowDown' && this.para.inclination > 0)
        this.para.inclination -= this.para.angleResolution
      else if (e.key === 'ArrowUp' && this.para.inclination < 180)
        this.para.inclination += this.para.angleResolution
