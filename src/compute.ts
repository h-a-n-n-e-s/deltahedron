...
export class Compute {
  ...
  initialize = async (objects: Array<Object>, timeStep: number, subSteps: number) => {
    ...
    triangles.meshBuffers = {
      vertexBuffer: this.triangleVertexBuffer,
      normalBuffer: this.triangleNormalBuffer,
      indexBuffer: indexBuffer,
      tangentBuffer: this.triangleTangentBuffer,
    }
    ...
    const trianglePipelineLayout = this.device.createPipelineLayout({
      bindGroupLayouts: [triangleBindGroupLayout],
    })
    ...
    this.trianglesPipeline = this.createCompPipe(
  ...
  setNewBallRodIndex = (newBallRodIndex: number) => {
    this.device.queue.writeBuffer(
      this.globalParameterBuffer,
      68,
      new Uint32Array([newBallRodIndex])
    )
  }
  ...
  setCount = (ballCount: number, rodCount: number, triangleCount: number) =>
    this.device.queue.writeBuffer(
      this.globalParameterBuffer,
      0,
      new Uint32Array([ballCount, rodCount, triangleCount])
    )

  setHalfEdgeBuffer = (halfEdges: U32Arr) =>
    this.device.queue.writeBuffer(this.halfEdgeBuffer, 0, halfEdges)

  setBallsBuffer = (index: number, ball: F32Arr, complete = true) => {
    if (complete) this.device.queue.writeBuffer(this.ballsBuffer, index * q * 4, ball)
    else {
  ...
  setRodsVisibility = (rodsVisible: boolean) =>
    this.device.queue.writeBuffer(
      this.globalParameterBuffer,
      56,
      new Uint32Array([rodsVisible ? 1 : 0])
    )
  ...
  setBallsVisibility = (ballsVisible: boolean) =>
    this.device.queue.writeBuffer(
      this.globalParameterBuffer,
      52,
      new Uint32Array([ballsVisible ? 1 : 0])
    )

  setMouseRayAndEye = (ray: F32Arr, eye: F32Arr) => {
