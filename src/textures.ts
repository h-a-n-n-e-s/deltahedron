...
export async function createHDRCubeMapTexture(device:GPUDevice, path:string) {
  return await textureFromHDRImages(
      device,
      [
        path+'/px.hdr',
        path+'/nx.hdr',
...
function copyHDRSourcesToTexture(device:GPUDevice, texture:GPUTexture, sources:Array<HDRImageData>) {
  sources.forEach((source, layer) => {
    device.queue.writeTexture(
      { texture: texture, origin:[0, 0, layer] },
      source.data,
      { bytesPerRow: source.width * source.data.BYTES_PER_ELEMENT * 4, rowsPerImage: source.height },
