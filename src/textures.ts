...
// download precomputed textures as binaries

// Add to textures.ts or a utility file
export function downloadTextureAsBinary(device: GPUDevice, texture: GPUTexture, filename: string) {
  // 1. Get buffer size
  const layers = texture.depthOrArrayLayers || 1
  const bytesPerPixel = 16 // rgba32float = 4 bytes * 4 channels
  const byteSize = texture.width * texture.height * layers * bytesPerPixel

  // 2. Create buffer to read GPU data
  ...
  commandEncoder.copyTextureToBuffer(
    { texture },
    { buffer: gpuBuffer, bytesPerRow: texture.width * bytesPerPixel, rowsPerImage: texture.height },
    { width: texture.width, height: texture.height, depthOrArrayLayers: layers }
  )
  device.queue.submit([commandEncoder.finish()])

  // 4. Read and Download
  gpuBuffer.mapAsync(GPUMapMode.READ).then(() => {
