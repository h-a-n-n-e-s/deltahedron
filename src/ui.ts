  ...
  constructor(buttonList: Array<{ name: string; func: (on: boolean) => void }>, isOnTop = true) {
    for (const entry of buttonList) {
      ...
      butt.onPush(() => {
        ...
        } // butt was active before
        else this.activeButton = undefined
        entry.func(butt.on)
      })
...
export class InfoSlider {
  ...
  constructor(
    ...
    if (prefix.includes('.svg')) {
      // use .svg file instead of string
      this.prefix = ''
...
export class Selection {
  ...
  constructor(
    options: IterableIterator<MIDIInput>,
    cangeCallback?: (e: Event) => void,
    ...
    for (let option of options) {
      ...
      // for debug
      if (option.name == 'microKEY2 KEYBOARD') {
        // 'minilogue xd KBD/KNOB'
